        ...
        jimuUtils.combineRadioCheckBoxWithLabel(this.limitToMapExtent, this.limitToMapExtentLabel);
			...
			_onSearchOptionChecked: function(evt) {
				...
				if (this._selectedOption) {
					...
					var filterLabel = domConstruct.create("label", {"innerHTML": this._selectedOption.label}); 
					...
					switch(this._selectedOption.input) {
						case "single":
							switch(this._selectedOption.dataType) {
								case "text": 
								case "number":
									this.filterValue = domConstruct.create("input", {"type": "text", "style":"width:175px"}); 
									this.filterInput.appendChild(this.filterValue);
									on(this.filterValue, "keyup", lang.hitch(this, this._onFilterValueChanged));
									break;
								case "date":
							}
							break;
						case "multiple":
							switch(this._selectedOption.dataType) {
								case "text": 
								case "number":
									this.filterValue = domConstruct.create("textarea", {"row": "5", "style":"width:175px"}); 
									this.filterInput.appendChild(this.filterValue);
									break;
								case "date":
							}
			...
			_fetchPartialMatches: function(textInput) {
				...
				if (this._selectedOption) {
					...
					/*
					// ignore the limit by the map extent
